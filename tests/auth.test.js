...
describe('Auth', () => {
    ...
    describe('API', () => {
        it('(register) should not create an account, already exist', async () => {
            let ret = await viatransit.API.register('test@viatransit.fr', 'Test12345');
        ...
        it('(login) should receive jwt_token', async () => {
            const token = await viatransit.API.login('test@viatransit.fr', 'Test12345');
        ...
        it("(login) should not receive jwt_token, wrong password", async () => {
            const ret = await viatransit.API.login('test@viatransit.fr', 'noop');
